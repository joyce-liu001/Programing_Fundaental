...
int sum(int num_lines, int num_digits, int array[MAX_SIZE][MAX_SIZE]) {
    ...
    while (colCounter >= 0) {
        ...
        int sum_col = 0;
        int final;
        while (rowCounter < num_lines) {
            sum_col = sum_col + array[rowCounter][colCounter];
            rowCounter++;
        }
        int test = sum_col + carryAccross;
        if (test > 9) {
            final = test % 10;
        } else { 
            final = test;
        ...
        array[rowCounter][colCounter] = final;
        colCounter--;
        carryAccross = test / 10;
    }
